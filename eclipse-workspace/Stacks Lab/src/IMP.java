...
class IMP<PixelRegion> implements MouseListener{
   JFrame frame;
   JPanel mp;
   JButton start;
   JScrollPane scroll;
  ...
  private void fun2()
  {
	  ...
	  Stack<PixelRegion> frat = new Stack<PixelRegion>();			//Pixel Region stack
	  int []Color = getPixelArray(picture[colorY][colorX]);			
	  PixelRegion.setThreshold(Color);								//creates threshold for testing pixels
	  PixelRegion start = new PixelRegion(colorY, colorX, Color);
	  frat.push(start);
	  
	  while(!frat.empty())
	  {
		  PixelRegion top = frat.peek();
		  PixelRegion test = top;
		  int iter = 1;
		  
		  switch(iter)			//checking at surrounding pixels
		  {
		  case 1:
			  test.setX(test.getX()-1);
			  test.setY(test.getY()-1);
			  test.setRGB(getPixelArray(picture[test.getY()][test.getX()])); 
			  if(test.testTresh() && test.checked())							//passed threshold and pixel has not been checked
			  {
				  frat.push(test);												//puts pixel in threshold on top the stack then makes then the pixel black
				  picture[test.getY()][test.getX()] = getPixels(test.getRGB());				
			  	break;															//breaks once there is a pixel in the threshold then returns to the top of the while loop
			  }
		  case 2:
			  test.setX(test.getX());
			  test.setY(test.getY()+1);
			  test.setRGB(getPixelArray(picture[test.getY()][test.getX()]));
			  if(test.testTresh() && test.checked())
			  {
				  frat.push(test);
				  picture[test.getY()][test.getX()] = getPixels(test.getRGB());
			  	break;
		  ...
		  default:																	//default case will pop the pixel off the stack once the surrounding pixels are not in the threshold
			  frat.pop();
			  break;
		  }
